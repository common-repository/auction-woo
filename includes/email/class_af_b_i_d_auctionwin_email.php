...
if ( ! class_exists('CT_B_I_D_Auction_Win') ) {
	class CT_B_I_D_Auction_Win extends WC_Email {


		public function __construct() {
			$this->id             = 'bid_auction_email_id'; // Unique ID to Store Emails Settings
			$this->title          = __('cloud technalogies Auction Win', 'auction-woo'); // Title of email to show in Settings
			$this->customer_email = true; // Set true for customer email and false for admin email.
			$this->description    = __('This will send a email when a customer win Auction', 'auction-woo'); // description of email
			$this->template_base  = BID_PLUGIN_DIR; // Base directory of template 
			$this->template_html  = 'includes/email-template/html/bid_auctionwin_html.php'; // HTML template path
			$this->template_plain = 'includes/email-template/plain/bid_auctionwin_plain.php'; // Plain template path
		...
		public function trigger( $post_id = 0 ) {
			...
			$this->placeholders['{bid_status}']         =   get_post_meta( $post_id, 'ct_bidding_status', true ) ? get_post_meta( $post_id, 'ct_bidding_status', true ) : 'Pending';
			$this->placeholders['{bid_amount}']         = isset($form_data['ct_bid_price']) ? $form_data['ct_bid_price'] : 0;
			$this->placeholders['{bid_date}']           =   get_the_date( $date_format, $post_id ) ;
			$this->placeholders['{product_name}']       =   get_the_title($product_id);
			$this->placeholders['{product_link}']       =   get_permalink($product_id);
			$this->setup_locale();
